 ...
 */
  ...
  getListOfMovies(): Promise<any> {
    let names = this.getListOfNames()
    let list = this.createAListOfPromises(names)
...
/*   getListOfMovies() {
    return this.getMovieNames()
      .then((names) => {
        return this.createPromiseList(names);
      })
  }

  createPromiseList(names) {
    let list = [];
    names.forEach(name => {
      let newpromise = this.getMovie(name);
      list.push(newpromise);
    });
    return Promise.all(list).then((movies) => { return movies })
  }
 */
}
